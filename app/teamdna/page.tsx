...
export default function TeamDNAPage() {
  ...
  return (
    <main className="min-h-screen relative overflow-x-hidden">
      {/* Background orbs */}
      <div
        className="orb w-[700px] h-[700px] -top-60 -left-60 opacity-20"
        style={{ background: 'radial-gradient(circle, #7c3aed, transparent)' }}
      />
      <div
        className="orb w-[500px] h-[500px] top-1/2 -right-60 opacity-15"
        style={{ background: 'radial-gradient(circle, #0891b2, transparent)' }}
      />

      {/* ── HERO ── */}
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-6 gap-6">
        <motion.p
          initial={{ opacity: 0, y: -16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="text-violet-400 font-mono text-sm tracking-[0.2em] uppercase"
        >
        ...
        <motion.div
          ...
          className="flex flex-col items-center gap-2 mt-8 text-white/40"
        >
          <span className="text-xs font-mono tracking-widest uppercase">scroll para ver mi perfil</span>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#0d0a1e] to-transparent" />
            </div>

            <div className="text-center">
              <p className="text-2xl font-black text-white">Danna Salamanca</p>
              <p className="text-violet-300 font-mono text-sm mt-1">Payments Operations Engineer · VAAS</p>
            </div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <span className="text-white/30 text-xs ml-3">danna_salamanca.sql</span>
            </div>

            {/* Query */}
            <div className="px-5 py-4 text-xs text-white/40 border-b border-white/5">
              <span className="text-violet-400">SELECT</span>
              <span className="text-white"> *</span>
              <span className="text-violet-400"> FROM</span>
              <span className="text-cyan-300"> danna_salamanca</span>
              <span className="text-white">;</span>
            </div>
            ...
            <div className="divide-y divide-white/5">
              {sqlRows.map((row, i) => (
                <motion.div
                  ...
                  className="flex items-start gap-3 px-5 py-2.5 hover:bg-white/3 transition-colors"
                >
                  <span className="text-white/30 text-xs w-4 flex-shrink-0 pt-0.5 select-none">{i + 1}</span>
                  <div className="flex items-start gap-2 min-w-0 flex-1">
                    <span className="text-blue-300 flex-shrink-0 text-xs">{row.campo}</span>
